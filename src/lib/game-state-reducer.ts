...
/**
 * Ensures a player number exists in the matchContext roster.
 * If the number isn't found, adds an anonymous entry with an autogenerated id.
 * Returns the (possibly updated) matchContext — callers must spread it into state.
 */
function ensurePlayerInRoster(
  matchContext: NonNullable<LiveState['matchContext']>,
  team: Team,
  playerNumber: string,
): NonNullable<LiveState['matchContext']> {
  const rosterKey = team === 'home' ? 'homeRoster' : 'awayRoster';
  const roster = matchContext[rosterKey];
  if (roster.some(p => p.number === playerNumber)) return matchContext;
  ...
  console.log(`[ensurePlayerInRoster] Adding anonymous player #${playerNumber} to ${team} roster with id ${newPlayer.id}`);
  return {
    ...matchContext,
    [rosterKey]: [...roster, newPlayer],
  };
}

/**
 * Ensures all player numbers from a goal log exist in the matchContext roster.
 */
function ensureGoalPlayersInRoster(
  matchContext: NonNullable<LiveState['matchContext']>,
  team: Team,
  goal: GoalLog,
): NonNullable<LiveState['matchContext']> {
  let mc = matchContext;
  if (goal.scorer?.playerNumber) mc = ensurePlayerInRoster(mc, team, goal.scorer.playerNumber);
  if (goal.assist?.playerNumber) mc = ensurePlayerInRoster(mc, team, goal.assist.playerNumber);
  if (goal.assist2?.playerNumber) mc = ensurePlayerInRoster(mc, team, goal.assist2.playerNumber);
  goal.positives?.forEach(p => { if (p?.playerNumber) mc = ensurePlayerInRoster(mc, team, p.playerNumber); });
  goal.negatives?.forEach(n => { if (n?.playerNumber) mc = ensurePlayerInRoster(mc, team, n.playerNumber); });
  return mc;
}
...
export const gameReducer = (state: GameState, action: GameAction): GameState => {
  ...
  switch (action.type) {
    ...
    case 'ADD_GOAL': {
      ...
      // Ensure all referenced players exist in matchContext roster
      let newMatchContext = live.matchContext;
      if (newMatchContext) {
        newMatchContext = ensureGoalPlayersInRoster(newMatchContext, teamScored, newGoal);
      }
      ...
      const mcRoster = newMatchContext ? (teamScored === 'home' ? newMatchContext.homeRoster : newMatchContext.awayRoster) : [];
    ...
    case 'ADD_PLAYER_SHOT': {
      ...
      // Ensure player exists in matchContext roster
      let shotMatchContext = state.live.matchContext;
      if (shotMatchContext) {
        shotMatchContext = ensurePlayerInRoster(shotMatchContext, team, playerNumber);
      }
    ...
    case 'ADD_PENALTY': {
      ...
      // Ensure player exists in matchContext roster
      let penaltyMatchContext = live.matchContext;
      if (penaltyMatchContext && !penaltyDef.isBenchPenalty) {
        penaltyMatchContext = ensurePlayerInRoster(penaltyMatchContext, team, playerNumber.toUpperCase());
      }
      ...
      if (!penaltyDef.isBenchPenalty) {
        ...
        if (!playerExists) {
          const penaltyRoster = penaltyMatchContext ? (team === 'home' ? penaltyMatchContext.homeRoster : penaltyMatchContext.awayRoster) : [];
      ...
      newState = {
        ...state, live: {
          ...
          attendance: newAttendance,
          matchContext: penaltyMatchContext,
        }
      };
    ...
    case 'RECORD_SHOOTOUT_ATTEMPT': {
      ...
      // Ensure player exists in matchContext roster
      let shootoutMatchContext = state.live.matchContext;
      if (shootoutMatchContext && attemptData.playerNumber) {
        shootoutMatchContext = ensurePlayerInRoster(shootoutMatchContext, team, attemptData.playerNumber);
      }
      ...
      newState = {
        ...
        live: {
          ...state.live,
          matchContext: shootoutMatchContext,
          shootout: {
            ...shootout,
