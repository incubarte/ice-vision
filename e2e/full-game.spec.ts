...
test.describe('Player Number Changes', () => {
  test.beforeEach(async ({ request }) => {
    const seedLive = readSeed('live.json');
    const liveForTest = {
      ...seedLive,
  ...
  // Helpers scoped to the home team card
  function homeCard(page: Page) {
    return page.locator('[class*="card"]').filter({ hasText: 'Test Home' }).filter({ hasText: 'presentes' });
  }

  function playerRow(page: Page, playerName: string) {
    return homeCard(page).locator('span').filter({ hasText: playerName }).locator('..');
  }

  function numberInput(page: Page, playerName: string) {
    return playerRow(page, playerName).locator('input[placeholder="#"]');
  }

  test('changing number to an unassigned number should not create a new player row', async ({ page }) => {
    test.setTimeout(30000);
    await page.goto('/controls');

    await page.getByRole('tab', { name: 'Jugadores' }).click();
    await expect(page.getByText('0/3 presentes').first()).toBeVisible({ timeout: 5000 });

    // Activate HOME PLAYER ONE (#10)
    await page.getByText('HOME PLAYER ONE').click();
    await expect(page.getByText('1/3 presentes').first()).toBeVisible({ timeout: 3000 });

    // Change HOME PLAYER ONE's number from 10 to 99
    const input = numberInput(page, 'HOME PLAYER ONE');
    await input.click({ force: true });
    await input.fill('99');
    await input.press('Enter');

    // Row count should remain 3 (not 4 from a duplicate)
    await expect(homeCard(page).locator('input[placeholder="#"]')).toHaveCount(3);

    // There should be no row with number 10
    await expect(homeCard(page).locator('input[value="10"]')).toHaveCount(0);

    // HOME PLAYER ONE should still be active (input enabled) and show number 99
    await expect(numberInput(page, 'HOME PLAYER ONE')).toBeEnabled();
    await expect(numberInput(page, 'HOME PLAYER ONE')).toHaveValue('99');
  });

  test('changing number to a colliding number should not duplicate name nor deactivate player', async ({ page }) => {
    test.setTimeout(30000);
    await page.goto('/controls');
    ...
    // Activate HOME PLAYER ONE (#10) only — HOME PLAYER TWO (#20) stays inactive
    await page.getByText('HOME PLAYER ONE').click();
    await expect(page.getByText('1/3 presentes').first()).toBeVisible({ timeout: 3000 });

    // Change HOME PLAYER ONE's number from 10 to 20 (collides with HOME PLAYER TWO)
    const input = numberInput(page, 'HOME PLAYER ONE');
    await input.click({ force: true });
    await input.fill('20');
    await input.press('Enter');
    ...
    await expect(numberInput(page, 'HOME PLAYER ONE')).toBeEnabled();
    await expect(numberInput(page, 'HOME PLAYER ONE')).toHaveValue('20');

    // HOME PLAYER TWO should still exist as inactive with number 20
    await expect(homeCard(page).getByText('HOME PLAYER TWO')).toBeVisible();
    await expect(numberInput(page, 'HOME PLAYER TWO')).toBeDisabled();
    await expect(numberInput(page, 'HOME PLAYER TWO')).toHaveValue('20');

    // Activate HOME PLAYER TWO
    await page.getByText('HOME PLAYER TWO').click();
    await expect(page.getByText('2/3 presentes').first()).toBeVisible({ timeout: 3000 });

    // Row count should remain 3
    await expect(homeCard(page).locator('input[placeholder="#"]')).toHaveCount(3);

    // Both players should be active with number 20
    await expect(numberInput(page, 'HOME PLAYER ONE')).toBeEnabled();
    await expect(numberInput(page, 'HOME PLAYER ONE')).toHaveValue('20');
    await expect(numberInput(page, 'HOME PLAYER TWO')).toBeEnabled();
    await expect(numberInput(page, 'HOME PLAYER TWO')).toHaveValue('20');
  });
});
